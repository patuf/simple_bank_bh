...
## Starting
You'll need to checkout the repository and pull the latest commit from its main branch.
...
* POST: **/rest/v1.0/account**: Creates a new account for an existing customer. If the customer doesn't exists,
returns a 404 error, otherwise, returns the created Account and HATEOAS links.
  
* GET: **/rest/v1.0/account/{accountId}**: Retrieves the account with ID *accountId*. If the account doesn't exists,
returns a 404 error, otherwise, returns the created Account and HATEOAS links.
...
* GET **/rest/v1.0/report/customers**: Retrieves a paged list of all customers with their total balance 
  ...
  specified with the standard request parameters *?size=N&page=M*. Default page size is 20 items.
...
* In-memory database is used (as required) for both normal operation and tests. Running tests with the services started will result
  in the data being wiped, and the services will need to be restarted to allow reinitialization with the prepopulated
  data from data.sql
...
* The reporting functionality is somewhat detached with the idea that it would ease the forking out a microservice
  dedicated to reporting. In a reporting microservice, it is acceptable for tables to be denormalized. Therefore
...
* The logical constitution of the microservices in this solution is not ideal and was purely dictated by
